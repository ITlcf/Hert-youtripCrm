<template>
  <div>
    <div id='maxdiv'>
      <el-form :inline="true" id="myfrom" :model="formInline" class="demo-form-inline">
        <el-form-item label="状态" prop="c_countryname">
          <el-select v-model="formInline.country.c_countryname" placeholder="请选择状态" id="queryCountry">
        ...
        <el-dialog class="formDiv1" title="新增框" :visible.sync="dialogADDFormVisible">
          <el-form :model="addRow" ref="addRow" label-width="100px" class="demo-ruleForm">
            <el-form-item label="签证国家" prop="c_countryname">
              <el-select v-model="addRow.addcountry.c_countryname" placeholder="请选择国家" id="addcountry">
            ...
            <el-form-item label="送签地" prop="vp_place" >
              <el-select v-model="addRow.addplace.vp_place" id="addplace" placeholder="请选择送签地">
            ...
            <el-form-item label="有效时间：" prop="v_starttime" >
              <el-select v-model="addRow.v_starttime" id="addstart" placeholder="请选择有效时间">
            ...
            <el-form-item label="停留时间：" prop="v_stoptime" >
              <el-select v-model="addRow.v_stoptime" id="addstop" placeholder="请选择停留时间">
                <el-option value="1"></el-option>
                <el-option value="2"></el-option>
                <el-option value="3"></el-option>
                <el-option value="4"></el-option>
            ...
            <el-form-item label="办理时长：" prop="v_handling" >
              <el-select v-model="addRow.v_handling" id="addhand" placeholder="请选择办理时长">
                <el-option value="1"></el-option>
                <el-option value="2"></el-option>
                <el-option value="3"></el-option>
                <el-option value="4"></el-option>
      ...
      <el-table :data="currentPageData" border style="width: 100%;" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="保险名称" width="200px">
          <template slot-scope="scope">
            <span>{{ scope.row.in_name}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="保险内容" width="400px" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <span>{{ scope.row. in_content}}</span>
          </template>
        </el-table-column>
        <el-table-column label="状态" width="100px">
          <template slot-scope="scope">
            <span v-if="scope.row.in_true==8" style="color:#5EB95E">{{ scope.row.or_stateName}}</span>
            <span v-else-if="scope.row.in_true==9" style="color:#b6bec9">{{ scope.row.or_stateName}}</span>
          </template>
        </el-table-column>
...
//        console.log(_this.currentPageData[i].id==mulp[i].id)  // 当前页的所有数据
    ...
    created(){
      this.axios.get("/api/insuranceList.do").then((data)=>{
