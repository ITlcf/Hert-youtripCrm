  ...
  <div>
    <div id='maxdiv'>
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item label="" prop="vp_place">
          <el-select v-model="formInline.place.vp_place" placeholder="请选择大洲" id="queryPlace">
      ...
      <el-table :data="currentPageData" border style="width: 100%;" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="大洲" width="80px">
          <template slot-scope="scope">
            <span>{{ scope.row.c_contientname}}</span>
          </template>
        </el-table-column>
        <el-table-column label="轮播图" width="250px">
          <template slot-scope="scope">
            <img class="imgsize" width="180" height="80" :src="'/api/'+scope.row.c_imgSrcsmall">
          </template>
        </el-table-column>
        <el-table-column label="描述" width="320px">
          <template slot-scope="scope">
...
//          console.log(row.id)
      ...
      handleSizeChange(val) {
        ...
        for(var i=0;i<parseInt(this.pageNum) && i<this.tableData.length;i++){
          this.tableData[i].c_imgSrcsmall=this.tableData[i].c_imgSrcsmall.split('/').slice(1,this.tableData.length).join('/')
...
//        console.log(_this.currentPageData[i].id==mulp[i].id)  // 当前页的所有数据
    ...
    created(){
      this.axios.get("/api/countryMsg.do").then((data)=>{
